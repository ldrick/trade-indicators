...
describe('sma', () => {
  test.each([{ p: NaN }, { p: Infinity }, { p: -Infinity }, { p: -1 }, { p: 0 }, { p: 1.5 }])(
    ...
    ({ p }) => {
      expect(sma([1], p)).toStrictEqual(left(new NotPositiveIntegerError('period')));
    },
  );

  it('fails if not enough data to calculate for period', () => {
    expect(sma([1, 2], 3)).toStrictEqual(left(new NotEnoughDataError(3, 3)));
  });

  test.each([{ v: [0, 0, NaN, 0] }, { v: [0, 0, Infinity, 0] }, { v: [0, 0, -Infinity, 0] }])(
    'fails if values contains a infinit value $v',
    ({ v }) => {
      expect(sma(v, 2)).toStrictEqual(left(new InfinitNumberError()));
    },
  );
  ...
  test.each(<{ v: NonEmptyArray<number>; p: number; r: number[] }[]>[
