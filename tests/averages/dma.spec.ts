...
describe('dma', () => {
  test.each([{ p: NaN }, { p: Infinity }, { p: -Infinity }, { p: -1 }, { p: 0 }, { p: 1.5 }])(
    ...
    ({ p }) => {
      expect(dma([], p)).toStrictEqual(left(new NotPositiveIntegerError('period')));
    },
  );
  ...
  test.each([{ v: [0, 0, NaN, 0] }, { v: [0, 0, Infinity, 0] }, { v: [0, 0, -Infinity, 0] }])(
    ...
    ({ v }) => {
      expect(dma(v, 2, 1)).toStrictEqual(left(new InfinitNumberError()));
    },
  );

  it('fails if not enough data to calculate for period', () => {
    expect(dma([1, 2], 3)).toStrictEqual(left(new NotEnoughDataError(3, 3)));
  });

  test.each([{ f: NaN }, { f: Infinity }, { f: -Infinity }])(
    'fails if factor is not a finit number $f',
    ({ f }) => {
      expect(dma([1, 2, 3], 2, f)).toStrictEqual(left(new InfinitNumberError()));
    },
  );
